...
void OutputWriterParaview::writeFile(double currentTime)
{
  ...
  for (int j = 0; j < nCells[1]+1; j++)
  {
    for (int i = 0; i < nCells[0]+1; i++, index++)
    {
      ...
      arrayPressure->SetValue(index, discretization_->p().bilinearInterpolation(x,y));
    }
  }
  ...
  for (int j = 0; j < nCells[1]+1; j++)
  {
    ...
    for (int i = 0; i < nCells[0]+1; i++, index++)
    {
      ...
      velocityVector[0] = discretization_->u().verticalInterpolation(x,y);
      velocityVector[1] = discretization_->v().horizontalInterpolation(x,y);
